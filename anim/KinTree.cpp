...
tVector cKinTree::CalcBodyPartVel(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
...
cKinTree::eBodyShape cKinTree::GetBodyShape(const Eigen::MatrixXd &body_defs, int part_id) {
    cKinTree::eBodyShape shape =
        static_cast<eBodyShape>(static_cast<int>(body_defs(part_id, cKinTree::eBodyParamShape)));
    return shape;
}
...
tVector cKinTree::GetBodyAttachTheta(const Eigen::MatrixXd &body_defs, int part_id) {
    tVector attach_theta =
        tVector(body_defs(part_id, eBodyParamAttachThetaX), body_defs(part_id, eBodyParamAttachThetaY),
...
tVector cKinTree::CalcWorldAngularVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
                                      const Eigen::VectorXd &vel, int parent_id, const tVector &attach_pt) {
    cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
    tVector pos = cKinTree::LocalToWorldPos(joint_mat, state, parent_id, attach_pt);
    cSpAlg::tSpTrans world_to_pt = cSpAlg::BuildTrans(pos);
...
int cKinTree::GetNumDof(const Eigen::MatrixXd &joint_mat) {
    ...
    int num_dof =
        cKinTree::GetParamOffset(joint_mat, num_joints - 1) + cKinTree::GetParamSize(joint_mat, num_joints - 1);
    return num_dof;
}
    ...
    for (int j = 1; j < num_joints; ++j) {
        ...
        if (joint_type == eJointTypeSpherical) {
            ...
            out_pose.segment(offset, size) =
                (1 - lerp) * pose0.segment(offset, size) + lerp * pose1.segment(offset, size);
        }
    }
}
