...
bool cKinCharacter::Init(const std::string &char_file) { return cCharacter::Init(char_file); }
...
void cKinCharacter::BuildAcc(Eigen::VectorXd &out_acc) const { CalcAcc(mTime, out_acc); }
...
bool cKinCharacter::LoadMotion(const std::string &motion_file) {
    ...
    if (succ) {
        cMotion::tBlendFunc blend_func = std::bind(&cKinCharacter::BlendFrames, this, std::placeholders::_1,
...
const std::shared_ptr<cKinController> &cKinCharacter::GetController() const { return mController; }
...
bool cKinCharacter::HasController() const { return mController != nullptr; }
...
void cKinCharacter::EnableVelUpdate(bool enable) { mEnableVelUpdate = enable; }
