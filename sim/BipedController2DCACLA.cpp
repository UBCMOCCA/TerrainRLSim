...
cBipedController2DCACLA::cBipedController2DCACLA()
    : cTerrainRLCharController(), cBipedController2D(), cBaseControllerCacla() {
...
void cBipedController2DCACLA::Init(cSimCharacter *character, const tVector &gravity, const std::string &param_file) {
    cBaseControllerCacla::Init(character);
    cBipedController2D::Init(character, gravity, param_file);
}

bool cBipedController2DCACLA::IsCurrActionCyclic() const { return false; }

void cBipedController2DCACLA::UpdateAction() {
...
void cBipedController2DCACLA::Update(double time_step) {

    cBipedController2D::Update(time_step);

    const cSimBiped2D::eJoint joints[] = {
        cSimBiped2D::eJointLeftHip, cSimBiped2D::eJointRightHip, cSimBiped2D::eJointLeftKnee,
...
double cBipedController2DCACLA::CalcReward() const {
    tVector target_vel = GetTargetVel();

    const double vel_reward_w = 0.8;
    const double torque_reward_w = 0.1;
    double vel_reward = 0;
    ...
    double reward = 0;
    reward += vel_reward_w * vel_reward + stumble_reward_w * stumble_reward + (torque_reward * torque_reward_w);
