...
void cGroundVar2D::Init(std::shared_ptr<cWorld> world, const tParams &params, const tVector &bound_min,
...
double cGroundVar2D::SampleHeight(const tVector &pos) const { return cGround::SampleHeight(pos); }
...
cGroundVar2D::eClass cGroundVar2D::GetGroundClass() const { return eClassVar2D; }
...
int cGroundVar2D::GetGridLength() const { return mSegments[0]->GetGridLength(); }
...
int cGroundVar2D::GetNumSegments() const { return gNumSegments; }

void cGroundVar2D::SetTerrainFunc(cTerrainGen2D::tTerrainFunc func) { mTerrainFunc = func; }
...
int cGroundVar2D::GetBlendParamSize() const { return static_cast<int>(cTerrainGen2D::eParamsMax); }

bool cGroundVar2D::OutsideGrid(const tVector &coord) const {
    return (coord[0] < 0 || coord[0] >= GetGridWidth() || coord[1] < 0 || coord[1] >= GetGridLength());
}
...
void cGroundVar2D::BuildSegment(int seg_id, double bound_min, double bound_max, eAlignMode align_mode,
    ...
    double new_bound_min =
        (align_mode == eAlignMin) ? (bound_min) : (bound_max - (num_verts - 1) * tSegment::gGridSpacingX);
...
double cGroundVar2D::GetMinX() const { return GetMinSegment()->GetMinX(); }

double cGroundVar2D::GetMidX() const { return GetMinSegment()->GetMaxX(); }

double cGroundVar2D::GetMaxX() const { return GetMaxSegment()->GetMaxX(); }
...
cGroundVar2D::tSegment::~tSegment() { this->Clear(); }
...
void cGroundVar2D::tSegment::Init(const std::shared_ptr<cWorld> &world, double min_x, double friction) {
    ...
    btHeightfieldTerrainShape *height_field = new btHeightfieldTerrainShape(
        grid_width, grid_length, mData.data(), static_cast<btScalar>(height_scale), static_cast<btScalar>(min_height),
...
bool cGroundVar2D::tSegment::IsEmpty() const { return mData.size() == 0; }
...
int cGroundVar2D::tSegment::GetGridWidth() const { return static_cast<int>(mData.size()) / GetGridLength(); }

int cGroundVar2D::tSegment::GetGridLength() const { return gGridLength; }

double cGroundVar2D::tSegment::GetWidth() const { return (GetGridWidth() - 1) * gGridSpacingX; }

double cGroundVar2D::tSegment::GetLength() const { return (GetGridLength() - 1) * gGridSpacingZ; }
...
tVector cGroundVar2D::tSegment::CalcGridCoord(const tVector &pos) const {
    ...
    if (coord[0] > -tol && coord[0] < w - 1 + tol && coord[1] > -tol && coord[1] < l - 1 + tol) {
        coord = ClampCoord(coord);
    }
...
bool cGroundVar2D::tSegment::OutsideGrid(const tVector &coord) const {
    return (coord[0] < 0 || coord[0] > GetGridWidth() - 1 || coord[1] < 0 || coord[1] > GetGridLength() - 1);
}
