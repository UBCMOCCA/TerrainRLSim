...
cJoint &cPDController::GetJoint() { return mChar->GetJoint(GetJointID()); }

const cJoint &cPDController::GetJoint() const { return mChar->GetJoint(GetJointID()); }

void cPDController::SetKp(double kp) { mParams[eParamKp] = kp; }

double cPDController::GetKp() const { return mParams[eParamKp]; }

double cPDController::GetTorqueLimit() const { return GetJoint().GetTorqueLimit(); }

double cPDController::GetForceLimit() const { return GetJoint().GetForceLimit(); }
...
void cPDController::SetUseWorldCoord(bool use) { mParams[eParamUseWorldCoord] = (use) ? 1 : 0; }

bool cPDController::UseWorldCoord() const { return mParams[eParamUseWorldCoord] != 0; }
...
void cPDController::GetTargetTheta(Eigen::VectorXd &out_theta) const {
    ...
    */
    ...
    if (UseWorldCoord() && joint_type == cKinTree::eJointTypeRevolute) {
        ...
        double theta_offset = axis_world.dot(axis_ref) * theta_world - axis_rel.dot(axis_ref) * theta_rel;
    ...
    if (UseWorldCoord() &&
        ((joint_type != cKinTree::eJointTypeRevolute) && (joint_type != cKinTree::eJointTypeSpherical))) {
...
int cPDController::GetJointDim() const { return mChar->GetParamSize(GetJointID()); }

int cPDController::GetJointID() const { return static_cast<int>(mParams[eParamJointID]); }

void cPDController::ApplyControlForces(const Eigen::VectorXd &tau) { mChar->ApplyControlForces(tau); }
