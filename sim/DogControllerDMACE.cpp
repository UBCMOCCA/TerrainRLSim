#include "DogControllerDMACE.h"

cDogControllerDMACE::cDogControllerDMACE() : cTerrainRLCharController(), cDogController(), cBaseControllerDMACE() {
    mExpParams.mBaseActionRate = 0.2;
    mExpParams.mNoise = 0.2;
}

cDogControllerDMACE::~cDogControllerDMACE() {}

void cDogControllerDMACE::Init(cSimCharacter *character, const tVector &gravity, const std::string &param_file) {
    cBaseControllerDMACE::Init(character);
    cDogController::Init(character, gravity, param_file);
}

void cDogControllerDMACE::Reset() {
    cBaseControllerDMACE::Reset();
    cDogController::Reset();
}

bool cDogControllerDMACE::IsCurrActionCyclic() const { return false; }

void cDogControllerDMACE::UpdateAction() {
    cBaseControllerDMACE::UpdateAction();
    cDogController::UpdateAction();
}

void cDogControllerDMACE::BuildBaseAction(int action_id, tAction &out_action) const {
    cDogController::BuildBaseAction(action_id, out_action);
    out_action.mID = AssignFragID(action_id);
}

void cDogControllerDMACE::ProcessCommand(tAction &out_action) {
    cBaseControllerDMACE::ProcessCommand(out_action);
    cDogController::ProcessCommand(out_action);
}

int cDogControllerDMACE::AssignFragID(int a_id) const {
...
void cDogControllerDMACE::BuildActorBias(int a_id, Eigen::VectorXd &out_bias) const {
    int num_ctrl_params = static_cast<int>(mCtrlParams.size());
