...
void cCtController::SetUpdatePeriod(double period) { mUpdatePeriod = period; }

double cCtController::GetUpdatePeriod() const { return mUpdatePeriod; }
...
int cCtController::GetNumActions() const { return 0; }

int cCtController::GetNumParams() const { return GetPoliActionSize(); }
...
bool cCtController::IsCurrActionCyclic() const { return false; }
...
int cCtController::GetGroundSampleRes() const { return mGroundSampleRes; }

void cCtController::SetGroundSampleRes(int sample_res) { mGroundSampleRes = sample_res; }

double cCtController::GetMaxViewDist() const { return mViewDist; }

double cCtController::GetMinViewDist() const { return mViewDistMin; }
...
void cCtController::UpdateBuildTau(double time_step, Eigen::VectorXd &out_tau) {
    ...
    for (int j = root_id + 1; j < num_joints; ++j) {
        ...
        out_tau.segment(param_offset, param_size) =
            mCurrAction.mParams.segment(retarget_offset - root_size, retarget_size);
    }
}
...
void cCtController::BuildJointActionOffsetScale(int joint_id, Eigen::VectorXd &out_offset,
