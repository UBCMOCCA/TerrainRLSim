...
const std::string gMiscParamsNames[cDogController::eMiscParamMax] = {"TransTime",  "Cv",          "BackForceX",
                                                                     "BackForceY", "FrontForceX", "FrontForceY"};

const std::string gStateParamNames[cDogController::eStateParamMax] = {"SpineCurve", "Shoulder", "Elbow",
                                                                      "Hip",        "Knee",     "Ankle"};
...
const cSimDog::eJoint gSpineJoints[] = {cSimDog::eJointSpine0, cSimDog::eJointSpine1, cSimDog::eJointSpine2,
                                        cSimDog::eJointSpine3, cSimDog::eJointTorso};
...
void cDogController::Init(cSimCharacter *character, const tVector &gravity, const std::string &param_file) {
    ...
    bool succ = LoadParams(param_file);
    if (succ) {
...
bool cDogController::LoadParams(const std::string &param_file) {
...
void cDogController::Update(double time_step) { cTerrainRLCharController::Update(time_step); }

void cDogController::UpdateCalcTau(double time_step, Eigen::VectorXd &out_tau) {
...
void cDogController::UpdateApplyTau(const Eigen::VectorXd &tau) {
    mTau = tau;
...
void cDogController::TransitionState(int state) { cTerrainRLCharController::TransitionState(state); }
...
int cDogController::GetNumStates() const { return eStateMax; }
...
int cDogController::GetDefaultAction() const { return mDefaultAction; }
...
int cDogController::GetNumActions() const { return static_cast<int>(mActions.size()); }
...
void cDogController::FetchOptParamScale(Eigen::VectorXd &out_scale) const {
    ...
    for (int i = 0; i < eMiscParamMax; ++i) {
        ...
        } else if (i == eMiscParamBackForceX || i == eMiscParamBackForceY || i == eMiscParamFrontForceX ||
...
void cDogController::ReadParams(const std::string &file) { cTerrainRLCharController::ReadParams(file); }
...
void cDogController::SetOptParams(const Eigen::VectorXd &opt_params) { SetOptParams(opt_params, mCurrAction.mParams); }
...
double cDogController::CalcReward() const {
    ...
    double reward = 0;
    reward += vel_reward_w * vel_reward + stumble_reward_w * stumble_reward;

    return reward;
}

tVector cDogController::GetTargetVel() const { return tVector(4, 0, 0, 0); }

double cDogController::GetPrevCycleTime() const { return mPrevCycleTime; }

const tVector &cDogController::GetPrevDistTraveled() const { return mPrevDistTraveled; }
...
bool cDogController::HasCtrlParams() const { return mCtrlParams.size() > 0; }
...
void cDogController::ApplyFeedback() {
    const cSimDog::eJoint joints[] = {cSimDog::eJointHip, cSimDog::eJointShoulder};
    const int num_joints = sizeof(joints) / sizeof(joints[0]);

    const cSimDog::eJoint end_effectors[] = {cSimDog::eJointToe, cSimDog::eJointFinger};

    const eStateParam params[] = {eStateParamHip, eStateParamShoulder};
    ...
    for (int j = 0; j < num_joints; ++j) {
        ...
        if (!contact) {
            ...
            theta_vec[0] = corrected_theta;
            mImpPDCtrl.SetTargetTheta(joint_id, theta_vec);
        }
    }
}
...
void cDogController::ApplyVirtualForces(Eigen::VectorXd &out_tau) {
    for (int e = 0; e < gNumEndEffectors; ++e) {
        ...
        if (active_effector) {
            ...
            while (curr_id != cSimDog::eJointRoot && curr_id != cSimDog::eJointTorso) {
...
const cDogController::tStateDef &cDogController::GetCurrStateDef() const { return gStateDefs[mState]; }
...
void cDogController::SetStateParams(const tStateParams &params) {
    for (int i = 0; i < gNumSpineJoints; ++i) {
        ...
        mImpPDCtrl.SetTargetTheta(joint, params.segment(eStateParamSpineCurve, 1));
    }
    mImpPDCtrl.SetTargetTheta(cSimDog::eJointShoulder, params.segment(eStateParamShoulder, 1));
    mImpPDCtrl.SetTargetTheta(cSimDog::eJointElbow, params.segment(eStateParamElbow, 1));
    mImpPDCtrl.SetTargetTheta(cSimDog::eJointHip, params.segment(eStateParamHip, 1));
    mImpPDCtrl.SetTargetTheta(cSimDog::eJointKnee, params.segment(eStateParamKnee, 1));
    mImpPDCtrl.SetTargetTheta(cSimDog::eJointAnkle, params.segment(eStateParamAnkle, 1));
}
...
double cDogController::GetTransTime() const { return mCurrAction.mParams[eMiscParamTransTime]; }

double cDogController::GetCv() const { return mCurrAction.mParams[eMiscParamCv]; }
...
bool cDogController::IsActiveVFEffector(cSimDog::eJoint joint_id) const {
    bool valid_effector =
        ((mState == eStateBackStance || mState == eStateExtend) && joint_id == cSimDog::eJointToe) ||
        ((mState == eStateFrontStance || mState == eStateGather) && joint_id == cSimDog::eJointFinger);
...
const std::string &cDogController::GetStateName(eState state) const { return gStateDefs[state].mName; }

const std::string &cDogController::GetStateParamName(eStateParam param) const { return gStateParamNames[param]; }
...
void cDogController::BuildStateParamsFromPose(const Eigen::VectorXd &pose, tStateParams &out_params) {
    const cSimDog::eJoint ctrl_joints[] = {cSimDog::eJointShoulder, cSimDog::eJointElbow, cSimDog::eJointHip,
                                           cSimDog::eJointKnee, cSimDog::eJointAnkle};
...
const Eigen::VectorXd &cDogController::GetCtrlParams(int ctrl_id) const { return mCtrlParams[ctrl_id]; }
...
void cDogController::ApplyAction(int action_id) { cTerrainRLCharController::ApplyAction(action_id); }
...
void cDogController::PostProcessAction(tAction &out_action) const { PostProcessParams(out_action.mParams); }
...
bool cDogController::HasCommands() const { return !mCommands.empty(); }
...
void cDogController::DebugPrintAction(const tAction &action) const {
    printf("Action ID: %i\n", action.mID);
