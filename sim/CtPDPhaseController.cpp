...
cCtPDPhaseController::cCtPDPhaseController() : cCtController(), cCtPhaseController(), cCtPDController() {}
...
void cCtPDPhaseController::Init(cSimCharacter *character, const tVector &gravity, const std::string &param_file) {
    ...
    // hack to default the num samples to 0 because thats what all the old args files are expecting and TerrainRLChar
    // defaults to 200 for some reason...
...
int cCtPDPhaseController::GetNumGroundSamples() const { return cTerrainRLCharController::GetNumGroundSamples(); }
