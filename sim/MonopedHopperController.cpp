...
const std::string gMiscParamsNames[cMonopedHopperController::eMiscParamMax] = {"MagicGain"};

const std::string gStateParamNames[cMonopedHopperController::eStateParamMax] = {"Hip", "Knee"};
...
const cSimMonopedHopper::eJoint gSpineJoints[] = {cSimMonopedHopper::eJointHip, cSimMonopedHopper::eJointKnee};
...
void cMonopedHopperController::Update(double time_step) { cTerrainRLCharController::Update(time_step); }
...
void cMonopedHopperController::TransitionState(int state) { cTerrainRLCharController::TransitionState(state); }
...
int cMonopedHopperController::GetNumStates() const { return eStateMax; }
...
int cMonopedHopperController::GetDefaultAction() const { return mDefaultAction; }
...
int cMonopedHopperController::GetNumActions() const { return static_cast<int>(mActions.size()); }
...
void cMonopedHopperController::ReadParams(const std::string &file) { cTerrainRLCharController::ReadParams(file); }
...
double cMonopedHopperController::GetPrevCycleTime() const { return mPrevCycleTime; }

const tVector &cMonopedHopperController::GetPrevDistTraveled() const { return mPrevDistTraveled; }
...
bool cMonopedHopperController::HasCtrlParams() const { return mCtrlParams.size() > 0; }
...
void cMonopedHopperController::UpdateState(double time_step) {
    ...
    if (state.mName == "Ascent") {
        ...
        } else if ((_previousBody_Y > 0.0) && (Body_Y < 0.0) && (!contact)) { // starting to fall
            advance_state = true;
        }
    }
...
void cMonopedHopperController::UpdatePDCtrls(double time_step) { mImpPDCtrl.Update(time_step); }
...
void cMonopedHopperController::ApplyFeedback(Eigen::VectorXd &out_tau) {
    ...
    if (state.mName == "DownStance" || state.mName == "UpStance"
        ...
        // std::cout << "Standing: root theta " << root_out_theta << "root axis " << root_out_axis.transpose() << "
        // corrected theta for thigh: " << corrected_theta << " around axis " << out_axis.transpose() << std::endl;
        ...
        // mImpPDCtrl.SetTargetTheta(cSimMonopedHopper::eJointHip, corrected_theta);
        // std::cout << "Flying: root theta " << root_out_theta << "root axis " << root_out_axis.transpose() << "
        // corrected theta for thigh: " << corrected_theta << " around axis " << out_axis.transpose() << std::endl;
        root_out_theta *= -1.0f;
...
void cMonopedHopperController::ApplyVirtualForces(Eigen::VectorXd &out_tau) {
    ...
    // std::cout << "Hip Kp: " << pd_hip.GetKp() << " Kd: " << pd_hip.GetKd() << " torqueLimit "<<
    // pd_hip.GetTorqueLimit() << std::endl;
    ...
    std::cout << "Hip angle: " << out_theta << " out axis: " << out_axis.transpose() << " error " << error << "
    hip_torque " << out_tau(3) << std::endl;
    */
    /*
    double pris_delta = knee.CalcDisplacementPrismatic();
    ...
    */
    ...
    /*
    btHingeConstraint * btHip =
    static_cast<btHingeConstraint*>(this->mChar->GetJoint(cSimMonopedHopper::eJointHip).GetConstraintHandle().mCons);
    btSliderConstraint * btKnee =
    static_cast<btSliderConstraint*>(this->mChar->GetJoint(cSimMonopedHopper::eJointKnee).GetConstraintHandle().mCons);
    ...
    btHip->enableAngularMotor(true, -out_tau(3)/120.0, max_torque_hip);// Must have been created with axis (0,0-1), into
    screen
    */
...
cMonopedHopperController::tStateParams cMonopedHopperController::GetCurrParams() const {
    cMonopedHopperController::tStateParams params =
        mCurrAction.mParams.segment(eMiscParamMax + mState * eStateParamMax, eStateParamMax);
    return params;
}
...
bool cMonopedHopperController::IsActiveVFEffector(cSimMonopedHopper::eJoint joint_id) const {
    bool valid_effector =
        ((mState == eStateDownStance || mState == eStateUpStance) && joint_id == cSimMonopedHopper::eJointKnee);
...
const std::string &cMonopedHopperController::GetStateName(eState state) const { return gStateDefs[state].mName; }
...
double cMonopedHopperController::GetGetMagicGain() const { return mCurrAction.mParams[eMiscParamMagicGain]; }
...
const Eigen::VectorXd &cMonopedHopperController::GetCtrlParams(int ctrl_id) const { return mCtrlParams[ctrl_id]; }
...
void cMonopedHopperController::ApplyAction(int action_id) { cTerrainRLCharController::ApplyAction(action_id); }
...
void cMonopedHopperController::PostProcessAction(tAction &out_action) const { PostProcessParams(out_action.mParams); }
...
bool cMonopedHopperController::HasCommands() const { return !mCommands.empty(); }
...
int cMonopedHopperController::GetPoliStateSize() const { return cTerrainRLCharController::GetPoliStateSize(); }
