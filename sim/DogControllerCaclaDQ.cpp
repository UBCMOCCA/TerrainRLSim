...
tVector cDogControllerCaclaDQ::GetTargetVel() const { return tVector(_target_vel, 0, 0, 0); }

void cDogControllerCaclaDQ::Update(double time_step) {
    cDogControllerCacla::Update(time_step);

    const cSimDog::eJoint joints[] = {cSimDog::eJointHip,
...
void cDogControllerCaclaDQ::updateInternalState(Eigen::VectorXd &state) { this->_target_vel = state(0); }
