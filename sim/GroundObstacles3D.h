...
class cGroundObstacles3D : public cGroundTrail3D {
  ...
  protected:
    ...
    virtual void BuildSlabHeighData(const tVector &bound_min, const tVector &bound_max, std::vector<float> &out_data,
                                    std::vector<int> &out_flags);

    virtual void CullSegs(const tVector &bound_min, const tVector &bound_max);
    virtual void RasterizeSeg(const tTrailSeg &seg, const tVector &bound_min, const tVector &bound_max,
