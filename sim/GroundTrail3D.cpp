...
void cGroundTrail3D::Init(std::shared_ptr<cWorld> world, const tParams &params, const tVector &bound_min,
...
cGroundTrail3D::eClass cGroundTrail3D::GetGroundClass() const { return eClassTrail3D; }

int cGroundTrail3D::GetNumSegs() const { return static_cast<int>(mTrail.size()); }

const cGroundTrail3D::tTrailSeg &cGroundTrail3D::GetSeg(int i) const { return mTrail[i]; }
...
void cGroundTrail3D::RasterizeSeg(const tTrailSeg &seg, const tVector &bound_min, const tVector &bound_max,
    ...
    if (cMathUtil::IntersectAABBXZ(strip_aabb_min, strip_aabb_max, bound_min, bound_max)) {
        ...
        for (int j = j_min; j <= j_max; ++j) {
            for (int i = i_min; i <= i_max; ++i) {
                tVector pos =
                    tVector(origin[0] + (i * size[0]) / (res_x - 1), 0, origin[2] + (j * size[2]) / (res_z - 1), 0);
