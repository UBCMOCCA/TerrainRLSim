...
void cRBDUtil::BuildEndEffectorJacobian(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose, int joint_id,
...
Eigen::MatrixXd cRBDUtil::MultJacobianEndEff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &q,
...
Eigen::MatrixXd cRBDUtil::ExtractEndEffJacobian(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &J,
                                                int joint_id) {
    int curr_id = joint_id;
    ...
    for (int j = num_joints - 1; j >= 0; --j) {
        if (cKinTree::IsValidBody(body_defs, j)) {
            ...
            cSpAlg::tSpTrans body_world_trans =
                cSpAlg::CompTrans(cSpAlg::InvTrans(world_child_trans), cSpAlg::MatToTrans(body_joint_mat));
...
cSpAlg::tSpVec cRBDUtil::CalcJointWorldVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
...
cSpAlg::tSpVec cRBDUtil::CalcWorldVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose,
                                      const Eigen::VectorXd &vel, int joint_id) {
    Eigen::MatrixXd J;
...
tVector cRBDUtil::CalcCoMVel(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
...
void cRBDUtil::CalcCoM(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs, const Eigen::VectorXd &pose,
...
cSpAlg::tSpTrans cRBDUtil::BuildParentChildTransform(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose,
                                                     int j) {
...
cSpAlg::tSpTrans cRBDUtil::BuildChildParentTransform(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose,
                                                     int j) {
