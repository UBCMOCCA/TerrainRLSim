...
void WeightedEuclideanLossLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
...
void WeightedEuclideanLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
...

    {
        ...
        if (propagate_down[i]) {
            ...
            caffe_mul(bottom[i]->count(), this->diff_.cpu_data(), this->diff_.cpu_data(),
