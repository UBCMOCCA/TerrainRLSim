...
int cTrajectory::GetNumAnchors() const { return static_cast<int>(mAnchors.cols()); }
...
Eigen::VectorXd cTrajectory::GetAnchor(int idx) const { return mAnchors.col(idx); }

void cTrajectory::SetAnchor(const Eigen::VectorXd &anchor, int idx) { mAnchors.col(idx) = anchor; }
