...
void cMotionDB::processMotionClips(const cBVHReader &BVHData) {
    ...
    tMatrix motionTransRoot = rootJoint->matrix * getTransformationForFrame(rootJoint->channel_start,
    rootJoint->channels_order, motionData.data, 0); tMatrix motionTransLeftThigh = leftThigh->matrix *
    getTransformationForFrame(leftThigh->channel_start, leftThigh->channels_order, motionData.data, 0); tMatrix
    motionTransLeftChin = leftChin->matrix * getTransformationForFrame(leftChin->channel_start,
    leftChin->channels_order, motionData.data, 0); motionTransLeftFoot = leftFoot->matrix *
    getTransformationForFrame(leftFoot->channel_start, leftFoot->channels_order, motionData.data, 0); std::cout << "root
    joint mat: " << rootJoint->matrix << std::endl; std::cout << "motion transformation mat: root: " << motionTransRoot
    << std::endl; std::cout << "motion transformation mat: left thigh: " << motionTransLeftThigh << std::endl; std::cout
    << "motion transformation mat: left chin" << motionTransLeftChin << std::endl; std::cout << "motion transformation
    mat: left foot" << motionTransLeftFoot << std::endl;
    ...
    */
    ...
    for (size_t f = 2; f < BVHData.GetNumFrames() && (f < 500); f++) {
        ...
        // JointData* leftFoot = getJoint("LeftFoot");
        // tMatrix motionTransRoot = rootJoint->matrix * getTransformationForFrame(rootJoint->channel_start,
        // rootJoint->channels_order, motionData.data, f); tMatrix motionTransLeftThigh = leftThigh->matrix *
        // getTransformationForFrame(leftThigh->channel_start, leftThigh->channels_order, motionData.data, f); tMatrix
        // motionTransLeftChin = leftChin->matrix * getTransformationForFrame(leftChin->channel_start,
        // leftChin->channels_order, motionData.data, f); Eigen::Vector3d ea = motionTransLeftChin.eulerAngles(0, 1, 2);
        // motionTransLeftChin.
        // tMatrix motionTransLeftFoot2 = leftFoot->matrix * getTransformationForFrame(leftFoot->channel_start,
        // leftFoot->channels_order, motionData.data, f); std::cout << "root joint mat: " << rootJoint->matrix <<
        // std::endl; std::cout << "motion transformation mat: root: " << motionTransRoot << std::endl; std::cout <<
        // "motion transformation mat: left thigh: " << motionTransLeftThigh << std::endl; std::cout << "motion
        // transformation mat: left chin" << motionTransLeftChin << std::endl; std::cout << "motion transformation mat:
        // left foot" << motionTransLeftFoot2 << std::endl;
