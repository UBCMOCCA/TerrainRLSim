...
cPerlinNoise::cPerlinNoise() { mRepeat = gPermLen; }

cPerlinNoise::~cPerlinNoise() {}

int cPerlinNoise::Inc(int x) { return (x + 1) % mRepeat; }
...
double cPerlinNoise::Eval(tVector p) {
    ...
    double x1 = cMathUtil::Lerp(u, Grad(aaa, xf, yf, zf), Grad(baa, xf - 1, yf, zf));
    double x2 = cMathUtil::Lerp(u, Grad(aba, xf, yf - 1, zf), Grad(bba, xf - 1, yf - 1, zf));
    double y1 = cMathUtil::Lerp(v, x1, x2);

    double x3 = cMathUtil::Lerp(u, Grad(aab, xf, yf, zf - 1), Grad(bab, xf - 1, yf, zf - 1));
    double x4 = cMathUtil::Lerp(u, Grad(abb, xf, yf - 1, zf - 1), Grad(bbb, xf - 1, yf - 1, zf - 1));
