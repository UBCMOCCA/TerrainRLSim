...
cNeuralNetLearner::~cNeuralNetLearner() { mTrainer->UnregisterLearner(this); }
...
int cNeuralNetLearner::GetIter() const { return mIter; }

int cNeuralNetLearner::GetNumTuples() const { return mNumTuples; }

int cNeuralNetLearner::GetStage() const { return mStage; }
...
const cNeuralNet *cNeuralNetLearner::GetNet() const { return mNet; }

void cNeuralNetLearner::LoadNet(const std::string &net_file) { mNet->LoadNet(net_file); }

void cNeuralNetLearner::LoadSolver(const std::string &solver_file) { mNet->LoadSolver(solver_file); }
...
bool cNeuralNetLearner::HasNet() const { return mNet != nullptr; }

bool cNeuralNetLearner::IsDone() const { return mTrainer->IsDone(); }

void cNeuralNetLearner::ResetExpBuffer() { mTrainer->ResetExpBuffer(); }
