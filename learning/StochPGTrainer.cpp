...
void cStochPGTrainer::SetNoiseParams(const tNoiseParams &params) { mNoiseParams = params; }

const cStochPGTrainer::tNoiseParams &cStochPGTrainer::GetNoiseParams() const { return mNoiseParams; }
...
bool cStochPGTrainer::CheckActorTD(double td) const { return cCaclaTrainer::CheckActorTD(td); }

void cStochPGTrainer::BuildActorProblemY(const std::vector<int> &tuple_ids, const Eigen::MatrixXd &X,
...
void cStochPGTrainer::ApplyEntropy(const Eigen::MatrixXd &X, cNeuralNet::tProblem &out_prob) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        curr_thread = std::thread(&cStochPGTrainer::ApplyEntropyWorker, this, row_idx_beg, row_idx_end, &X,
                                  worker_net.get(), &out_prob);
    }
...
void cStochPGTrainer::ApplyEntropyWorker(int row_idx_min, int row_idx_max, const Eigen::MatrixXd *X, cNeuralNet *net,
                                         cNeuralNet::tProblem *out_prob) {
...
void cStochPGTrainer::ApplyEntropyHelper(int row_idx, const Eigen::MatrixXd *X, cNeuralNet *net,
                                         cNeuralNet::tProblem *out_prob) {
...
int cStochPGTrainer::GetNoiseInputOffset() const { return mNoiseParams.mInputOffset; }

int cStochPGTrainer::GetNoiseInputSize() const { return mNoiseParams.mInputSize; }
