...
void cAsyncTrainer::SetInputOffsetScaleType(const std::vector<cNeuralNet::eOffsetScaleType>& scale_types)
{
	for (int i = 0; i < GetNumTrainers(); ++i)
	{
		mTrainers[i]->SetInputOffsetScaleType(scale_types);
	}
}
...
void cAsyncTrainer::SetIntOutputCallback(tCallbackFunc func)
{
	for (int i = 0; i < GetNumTrainers(); ++i)
	{
		mTrainers[i]->SetIntOutputCallback(func);
	}
}

void cAsyncTrainer::OutputIntermediate()
{
	if (mTrainers.size() > 0)
	{
		mTrainers[0]->OutputIntermediate();
	}
}
...
void cAsyncTrainer::OutputIntermediateModel(const std::string& filename) const
{
	if (mTrainers.size() > 0)
	{
		mTrainers[0]->OutputIntermediateModel(filename);
	}
}
