...
class cMACETrainer : public cNeuralNetTrainer {
  public:
    ...
    static void GetFrag(const Eigen::VectorXd &params, int num_frags, int frag_size, int a_idx,
                        Eigen::VectorXd &out_action);
    static void SetFrag(const Eigen::VectorXd &frag, int a_idx, int num_frags, int frag_size,
  ...
  protected:
    ...
    virtual void BuildActorProblemY(int num_data, const std::vector<int> &tuple_ids, const Eigen::MatrixXd &X,
                                    cNeuralNet::tProblem &out_prob);
