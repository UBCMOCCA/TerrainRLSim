...
cACLearner::cACLearner(const std::shared_ptr<cNeuralNetTrainer> &trainer) : cNeuralNetLearner(trainer) {
    mCriticNet = nullptr;
}
...
void cACLearner::SetNet(cNeuralNet *net) { SetActorNet(net); }

const cNeuralNet *cACLearner::GetNet() const { return GetActorNet(); }
...
const cNeuralNet *cACLearner::GetActorNet() const { return mNet; }
...
const cNeuralNet *cACLearner::GetCriticNet() const { return mCriticNet; }

void cACLearner::LoadNet(const std::string &net_file) { LoadActorNet(net_file); }

void cACLearner::LoadSolver(const std::string &solver_file) { LoadActorSolver(solver_file); }
...
void cACLearner::LoadActorNet(const std::string &net_file) { mNet->LoadNet(net_file); }

void cACLearner::LoadActorSolver(const std::string &solver_file) { mNet->LoadSolver(solver_file); }
...
bool cACLearner::HasCriticNet() const { return mCriticNet != nullptr; }
