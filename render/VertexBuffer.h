...
class cVertexBuffer {
  public:
    ...
    cVertexBuffer() : mRenderID(0), mRenderState(NULL), mLocalData(NULL), mAttrInfo(NULL), mNumAttr(0), mSize(0) {}
    cVertexBuffer(GLuint bufferID, cRenderState &r_state)
        : mRenderID(bufferID), mRenderState(&r_state), mLocalData(NULL), mAttrInfo(NULL), mNumAttr(0), mSize(0) {}
    cVertexBuffer(const cVertexBuffer &old)
        : mRenderID(old.mRenderID), mRenderState(old.mRenderState), mNumAttr(old.mNumAttr), mSize(old.mSize) {
