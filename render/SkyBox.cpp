...
void cSkyBox::Init() {
    ...
    // normalize direction
    float sun_dir_norm = sqrt(mSunDirection[0] * mSunDirection[0] + mSunDirection[1] * mSunDirection[1] +
                              mSunDirection[2] * mSunDirection[2]);
...
void cSkyBox::UpdateHosekModel() {
    ...
    cHosekSkyModel::CalculateXYZCoefandLM(mTurbidity, mAlbedo, sun_elevation, mHosekSkyCoefs, mHosekSkyRadiance);
...
void cSkyBox::PreethamSky(const tVector &dir, const tVector &sun_dir, float *output) {
    ...
    float cos_gamma = std::min(1.0, (dir[0] * sun_dir[0] + dir[1] * sun_dir[1] + dir[2] * sun_dir[2]));

    float X =
        mHosekSkyRadiance[0] * PrezeHosekLuminance(cos_theta, cos_gamma, mHosekSkyCoefs[0][0], mHosekSkyCoefs[0][1],
                                                   mHosekSkyCoefs[0][2], mHosekSkyCoefs[0][3], mHosekSkyCoefs[0][4],
                                                   mHosekSkyCoefs[0][5], mHosekSkyCoefs[0][6], mHosekSkyCoefs[0][7],
                                                   mHosekSkyCoefs[0][8]);

    float Y =
        mHosekSkyRadiance[1] * PrezeHosekLuminance(cos_theta, cos_gamma, mHosekSkyCoefs[1][0], mHosekSkyCoefs[1][1],
                                                   mHosekSkyCoefs[1][2], mHosekSkyCoefs[1][3], mHosekSkyCoefs[1][4],
                                                   mHosekSkyCoefs[1][5], mHosekSkyCoefs[1][6], mHosekSkyCoefs[1][7],
                                                   mHosekSkyCoefs[1][8]);

    float Z =
        mHosekSkyRadiance[2] * PrezeHosekLuminance(cos_theta, cos_gamma, mHosekSkyCoefs[2][0], mHosekSkyCoefs[2][1],
                                                   mHosekSkyCoefs[2][2], mHosekSkyCoefs[2][3], mHosekSkyCoefs[2][4],
                                                   mHosekSkyCoefs[2][5], mHosekSkyCoefs[2][6], mHosekSkyCoefs[2][7],
                                                   mHosekSkyCoefs[2][8]);
...
float cSkyBox::PrezeHosekLuminance(float cos_theta, float cos_gamma, float A, float B, float C, float D, float E,
                                   float F, float G, float H, float I) {
    float gamma = std::acos(cos_gamma);
    float expM = std::exp(E * gamma);
    float rayM = std::cos(gamma) * std::cos(gamma);
    float mieM =
        (1.0 + std::cos(gamma) * std::cos(gamma)) / std::pow(std::abs(1.0 + I * I - 2.0 * I * std::cos(gamma)), 1.5);
    float zenith = std::sqrt(cos_theta);

    return (1.0 + A * std::exp(B / (cos_theta + 0.01))) * (C + D * expM + F * rayM + G * mieM + H * zenith);
}

const tVector &cSkyBox::GetSunDirection() const { return mSunDirection; }

const tVector &cSkyBox::GetSunColour() const { return mSunColour; }

const float **const cSkyBox::GetAmbientColour() const { return (const float **const)mAmbientColour; }
...
void cSkyBox::SetSunSize(double size) { mSunSize = size; }

cSkyBox::~cSkyBox(void) {}

void cSkyBox::setShaderRelativePath(std::string path) { this->mRelativePath = path; }
