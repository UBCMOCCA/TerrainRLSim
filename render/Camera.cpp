...
cCamera::cCamera(const tVector &pos, const tVector &focus, const tVector &up, double w, double h, double near_z,
                 double far_z) {
    mPosition = pos;
...
void cCamera::SetFocus(const tVector &focus) { mFocusDelta = focus - mPosition; }
...
void cCamera::TranslateFocus(const tVector &focus) { mPosition = focus - mFocusDelta; }
...
tVector cCamera::ScreenToWorldPos(const tVector &screen_pos) const {
    ...
    double focal_len = CalcFocalLen();
    tVector view_pos = tVector(0.5 * screen_pos[0] * w, 0.5 * screen_pos[1] * h, -focal_len, 1);
...
void cCamera::MouseMove(double x, double y) {
    if (mMouseDown) {
        ...
        if (mouse_mod & GLUT_ACTIVE_ALT) {
            ...
            // rotate
            tMatrix rot_mat =
                cMathUtil::RotateMat(tVector(0, 1, 0, 0), -M_PI * dx) * cMathUtil::RotateMat(right, M_PI * dy);
