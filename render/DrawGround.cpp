...
void cDrawGround::DrawRuler2D(const cGround *ground, const tVector &bound_min, const tVector &bound_max) {
    ...
    for (double x = min_x - std::fmod(min_x, gLineSpacing) - gLineSpacing; x < max_x + gLineSpacing;
         x += gLineSpacing) {
...
void cDrawGround::BuildMeshPlane(const cGround *ground, cDrawMesh *out_mesh) {
    ...
    float vert_data[vert_len] = {a[0], a[1], a[2], b[0], b[1], b[2], c[0], c[1], c[2],
                                 c[0], c[1], c[2], d[0], d[1], d[2], a[0], a[1], a[2]};

    float norm_data[norm_len] = {0.f, 1.f, 0.f, 0.f, 1.f, 0.f, 0.f, 1.f, 0.f,
                                 0.f, 1.f, 0.f, 0.f, 1.f, 0.f, 0.f, 1.f, 0.f};

    float coord_data[coord_len] = {coord_a[0], coord_a[1], coord_b[0], coord_b[1], coord_c[0], coord_c[1],
                                   coord_c[0], coord_c[1], coord_d[0], coord_d[1], coord_a[0], coord_a[1]};
    ...
    cMeshUtil::BuildDrawMesh(vert_data, vert_len, norm_data, norm_len, coord_data, coord_len, idx_data, idx_len,
                             out_mesh);
}
...
void cDrawGround::BuildMeshVar2D(const cGround *ground, cDrawMesh *out_mesh) {
    ...
    cMeshUtil::BuildDrawMesh(vert_data.data(), static_cast<int>(vert_data.size()), norm_data.data(),
                             static_cast<int>(norm_data.size()), coord_data.data(), static_cast<int>(coord_data.size()),
                             idx_data.data(), static_cast<int>(idx_data.size()), out_mesh);
}

void cDrawGround::BuildMeshVar3D(const cGround *ground, cDrawMesh *out_mesh) {
    assert(ground->GetGroundClass() == cGround::eClassVar3D || ground->GetGroundClass() == cGround::eClassHills3D ||
