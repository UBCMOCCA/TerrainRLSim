...
void SetupCamProjection() { gCamera.SetupGLProj(); }
...
void InitCamera() {
    gCamera = cCamera(gCameraPosition, gCameraFocus, gCameraUp, gViewWidth, gViewHeight, gViewNearZ, gViewFarZ);
    gCamera.SetProj(cCamera::eProjOrtho);
    ResizeCamera();
}

void ClearScenario() { gScenario = nullptr; }
...
void CopyFrame() { cDrawUtil::CopyTexture(*gIntermediateFrameBuffer, *gDefaultFrameBuffer); }

void UpdateIntermediateBuffer() {
    if (!gReshaping) {
        if (gWinWidth != gIntermediateFrameBuffer->GetWidth() || gWinHeight != gIntermediateFrameBuffer->GetHeight()) {
...
void Update(double time_elapsed) { UpdateScenario(time_elapsed); }
...
void InitOpenGl(void) {
    ...
    gDefaultFrameBuffer =
        std::unique_ptr<cTextureDesc>(new cTextureDesc(0, 0, 0, gWinWidth, gWinHeight, 1, GL_RGBA, GL_RGBA));
    gIntermediateFrameBuffer = std::shared_ptr<cTextureDesc>(
        new cTextureDesc(gWinWidth, gWinHeight, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, false));
}
...
void HackRaptorMeasurements() {
    ...
    right_thigh_theta_rel =
        (right_thigh_theta_rel > M_PI) ? -(2 * M_PI - right_thigh_theta_rel) : right_thigh_theta_rel;
    right_knee_theta_rel = (right_knee_theta_rel > M_PI) ? -(2 * M_PI - right_knee_theta_rel) : right_knee_theta_rel;
    right_ankle_theta_rel =
        (right_ankle_theta_rel > M_PI) ? -(2 * M_PI - right_ankle_theta_rel) : right_ankle_theta_rel;
