...
cDrawScenarioSimInteractive::cDrawScenarioSimInteractive(cCamera &cam) : cDrawScenarioTerrainRL(cam) {
...
void cDrawScenarioSimInteractive::UpdateScene(double time_elapsed) { ApplyUIForce(time_elapsed); }
...
tVector cDrawScenarioSimInteractive::GetDefaultCamFocus() const { return gCamFocus0; }

void cDrawScenarioSimInteractive::RayTest(const tVector &start, const tVector &end,
                                          cWorld::tRayTestResult &out_result) {
    return GetScene()->RayTest(start, end, out_result);
}

bool cDrawScenarioSimInteractive::ObjectSelected() const { return mSelectedObj != nullptr; }
...
void cDrawScenarioSimInteractive::ApplyUIForce(double time_step) {
    if (ObjectSelected()) {
        ...
        tPerturb perturb = tPerturb(tPerturb::ePerturbForce, mSelectedObj, mSelectObjLocalPos, force, time_step);
