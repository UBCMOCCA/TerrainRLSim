...
cDrawScenario::cDrawScenario(cCamera &cam) : mCam(cam) {
...
void cDrawScenario::Update(double time_elapsed) { UpdateCamera(); }
...
void cDrawScenario::Init() { EnableFilmstrip(false); }
...
cDrawScenario::eCamTrackMode cDrawScenario::GetTrackMode() const { return mCamTrackMode; }

cDrawScenario::eCamTrackMode cDrawScenario::GetTrackMode0() const { return mCamTrackMode0; }

void cDrawScenario::EnableFilmstrip(bool enable) { mEnableFilmStrip = enable; }
...
void cDrawScenario::UpdateCamera() {
    if (mCamTrackMode == eCamTrackModeXZ || mCamTrackMode == eCamTrackModeY || mCamTrackMode == eCamTrackModeXYZ) {
...
void cDrawScenario::UpdateCameraTracking() {
    ...
    } else if (mCamTrackMode == eCamTrackModeXZ || mCamTrackMode == eCamTrackModeY) {
        tVector track_pos = GetCamTrackPos();
...
void cDrawScenario::UpdateCameraStill() {
    ...
    bool snap_y =
        (track_pos[1] - cam_focus[1]) > 0.5 * cam_h - pad_y || (track_pos[1] - cam_focus[1]) < -(0.5 * cam_h - pad_y);
    ...
    if (snap_x || snap_y) {
        ...
        if ((pos_delta[1]) > 0.5 * cam_h - pad_y || (pos_delta[1]) < -(0.5 * cam_h - pad_y)) {
...
tVector cDrawScenario::GetCamTrackPos() const { return tVector::Zero(); }

tVector cDrawScenario::GetCamStillPos() const { return tVector::Zero(); }
...
void cDrawScenario::ResetCamera() {
    ...
    if (mCamTrackMode == eCamTrackModeXZ || mCamTrackMode == eCamTrackModeY) {
...
tVector cDrawScenario::GetDefaultCamFocus() const { return tVector::Zero(); }
...
std::string cDrawScenario::BuildTextInfoStr() const { return ""; }
