...
double cScenarioExpImitateTarget::CalcTargetReward(const tVector &tar_pos) const {
    ...
    if (!fallen) {
        ...
        reward = pose_w * pose_reward + vel_w * vel_reward + end_eff_w * end_eff_reward + root_w * root_reward +
                 com_w * com_reward + heading_w * heading_reward + target_pos_w * target_pos_reward;
    }
...
const tVector &cScenarioExpImitateTarget::GetTargetPos() const { return mTargetPos; }
...
void cScenarioExpImitateTarget::EnableRandTargetPos(bool enable) { mEnableRandTargetPos = enable; }

bool cScenarioExpImitateTarget::EnabledRandTargetPos() const { return mEnableRandTargetPos; }

double cScenarioExpImitateTarget::GetTargetResetDist() const { return mTargetResetDist; }

std::string cScenarioExpImitateTarget::GetName() const { return "Imitate Target Exploration"; }
...
bool cScenarioExpImitateTarget::CheckResetTarget() const {
    ...
    bool reset_target = (mRandTargetPosTimer <= 0) || (dist < dist_threshold * dist_threshold);
...
double cScenarioExpImitateTarget::CalcReward() const { return CalcTargetReward(mTargetPos); }
...
double cScenarioExpImitateTarget::GetRandTargetMaxDist() const { return gMaxTargetDist; }
