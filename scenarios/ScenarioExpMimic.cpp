...
const std::shared_ptr<cKinCharacter> &cScenarioExpMimic::GetKinChar() const { return mKinChar; }
...
void cScenarioExpMimic::EnableCoachActiveProb(bool enable) { mEnableCoachActiveProb = enable; }

void cScenarioExpMimic::SetCoachActiveProb(double prob) { mCoachActiveProb = prob; }

int cScenarioExpMimic::GetCharCtrlID() const { return mCharCtrlID; }

int cScenarioExpMimic::GetCoachCtrlID() const { return mCoachCtrlID; }

std::string cScenarioExpMimic::GetName() const { return "Mimic Exploration"; }
...
bool cScenarioExpMimic::BuildCoachController(const cTerrainRLCtrlFactory::tCtrlParams &params,
                                             std::shared_ptr<cTerrainRLCharController> &out_ctrl) {
    std::shared_ptr<cCharController> ctrl;
...
bool cScenarioExpMimic::EnableRandInitAction() const { return false; }
...
void cScenarioExpMimic::UpdateKinChar(double time_step) { mKinChar->Update(time_step); }
...
int cScenarioExpMimic::GetNumWarmupCycles() const { return gNumWarmupCycles; }
...
std::shared_ptr<const cNNController> cScenarioExpMimic::GetNNController() const {
    int char_ctrl_id = GetCharCtrlID();
    std::shared_ptr<cMixController> mix_ctrl = std::static_pointer_cast<cMixController>(mChar->GetController());
    std::shared_ptr<cNNController> ctrl =
        std::static_pointer_cast<cNNController>(mix_ctrl->GetController(char_ctrl_id));
    return ctrl;
}
...
double cScenarioExpMimic::CalcReward() const { return 0; }

bool cScenarioExpMimic::IsCoachWarmup() const { return mCoachWarmupTimer < mCouchWarmupTimeMax; }
...
double cScenarioExpMimic::GetEpisodeMaxTime() const { return mEpisodeMaxTime + mCouchWarmupTimeMax; }
