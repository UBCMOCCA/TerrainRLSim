...
cDrawScenarioSimChar::cDrawScenarioSimChar(cCamera &cam) : cDrawScenarioSimInteractive(cam) {
    mDrawInfo = false;
    mDrawPoliInfo = false;
    mEnableTrace = false;
    mDrawFeatures = false;
...
void cDrawScenarioSimChar::PauseSim(bool pause) { mPauseSim = pause; }
...
int cDrawScenarioSimChar::AddCharTrace(const std::shared_ptr<cSimCharacter> &character, const tVectorArr &cols) {
...
void cDrawScenarioSimChar::DrawMisc() const { DrawPerturbs(); }
...
void cDrawScenarioSimChar::DrawFeatures() const {
    ...
    cDrawSimCharacter::DrawCharFeatures(*(character.get()), *ground.get(), marker_size, vel_scale, pos_col, vel_col,
                                        GetVisOffset());
...
std::string cDrawScenarioSimChar::BuildTextInfoStr() const {
    ...
    sprintf(buffer, "Time: %.2fs\nPosition: (%.2f, %.2f, %.2f)\nVelocity: (%.2f, %.2f, %.2f)\n", time, com[0], com[1],
            com[2], com_vel[0], com_vel[1], com_vel[2]);
#else
...
void cDrawScenarioSimChar::Shutdown() { mScene->Shutdown(); }

std::string cDrawScenarioSimChar::GetName() const { return mScene->GetName(); }

const std::shared_ptr<cScenarioSimChar> &cDrawScenarioSimChar::GetScene() const { return mScene; }

std::string cDrawScenarioSimChar::GetOutputCharFile() const { return gOutputCharFile; }

void cDrawScenarioSimChar::OutputCharState(const std::string &out_file) const { mScene->OutputCharState(out_file); }

void cDrawScenarioSimChar::SpawnProjectile() { mScene->SpawnProjectile(); }

void cDrawScenarioSimChar::SpawnBigProjectile() { mScene->SpawnBigProjectile(); }
