...
std::string cScenarioTrainCaclaDV::GetName() const { return "Train CaclaDV"; }

void cScenarioTrainCaclaDV::SetupLearner(const std::shared_ptr<cSimCharacter> &character,
...
void cScenarioTrainCaclaDV::SetupTrainerForwardDynamicsOffsetScale(
    const std::shared_ptr<cTrainerInterface> &out_trainer) {
    ...
    if (!valid_init_model) {
        ...
        if (cacla_ctrl != nullptr) {
            ...
            Eigen::VectorXd fd_input_offset =
                Eigen::VectorXd::Ones(actor_input_offset.rows() + actor_output_offset.rows());
            Eigen::VectorXd fd_input_scale =
                Eigen::VectorXd::Ones(actor_input_offset.rows() + actor_output_offset.rows());
