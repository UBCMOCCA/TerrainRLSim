...
double cScenarioExpImitateStep::CalcRewardStep(const cBipedStepController3D::tStepPlan &step_plan) const {
    ...
    if (!fallen) {
        ...
        reward = pose_w * pose_reward + vel_w * vel_reward + end_eff_w * end_eff_reward + root_w * root_reward +
                 com_w * com_reward + heading_w * heading_reward;
...
const cBipedStepController3D::tStepPlan &cScenarioExpImitateStep::GetStepPlan() const { return mStepPlan; }

bool cScenarioExpImitateStep::EnableTargetPos() const { return mEnableTargetPos; }

void cScenarioExpImitateStep::EnableTargetPos(bool enable) { mEnableTargetPos = enable; }

std::string cScenarioExpImitateStep::GetName() const { return "Imitate Step Exploration"; }
...
bool cScenarioExpImitateStep::EnableUpdateStepPlan() const { return true; }

double cScenarioExpImitateStep::CalcReward() const { return CalcRewardStep(mStepPlan); }
...
void cScenarioExpImitateStep::SwatchEndEffectorPose(const std::vector<int> &end_effector,
...
void cScenarioExpImitateStep::ResetStepPlan() {
    ...
    eStance stance = GetStance();
    eStance swing = (stance == cBipedStepController3D::eStanceRight) ? cBipedStepController3D::eStanceLeft
...
double cScenarioExpImitateStep::CalcNextHeading(double heading) {
    ...
    if (EnableTargetPos()) {
        ...
        while (true) {
            ...
            tVector sample_pos =
                curr_center + sample_dist * tVector(std::cos(next_heading), 0, -std::sin(next_heading), 0);
