...
void cScenarioExp::SetBufferSize(int size) { mTupleBufferSize = size; }

void cScenarioExp::ResetTupleBuffer() { mTupleCount = 0; }

bool cScenarioExp::IsTupleBufferFull() const { return mTupleCount >= mTupleBufferSize; }

const std::vector<tExpTuple> &cScenarioExp::GetTuples() const { return mTupleBuffer; }

std::string cScenarioExp::GetName() const { return "Exploration"; }
...
double cScenarioExp::GetEpisodeMaxTime() const { return mEpisodeMaxTime; }
...
void cScenarioExp::IncCycleCount() { ++mCycleCount; }

void cScenarioExp::ClearReward(tExpTuple &out_tuple) const { out_tuple.mReward = 0; }
...
void cScenarioExp::SetEpisodeTimeLimExpLambda(double lambda) { mEpisodeTimeLimExpLambda = lambda; }

double cScenarioExp::GetEpisodeTimeLimExpLambda(double lambda) const { return mEpisodeTimeLimExpLambda; }
...
std::vector<std::string> &cScenarioExp::GetNetFiles() { return mCtrlParams.mNetFiles; }
...
void cScenarioExp::ClearFlags(tExpTuple &out_tuple) const { out_tuple.ClearFlags(); }
...
bool cScenarioExp::EnableRandInitAction() const { return true; }
...
bool cScenarioExp::IsFirstValidCycle() const { return mCycleCount == GetNumWarmupCycles() + 1; }
...
int cScenarioExp::GetNumWarmupCycles() const { return gNumWarmupCycles; }
...
bool cScenarioExp::EnableRandTimeLim() const { return mEnableRandEpisodeTimeLim; }
