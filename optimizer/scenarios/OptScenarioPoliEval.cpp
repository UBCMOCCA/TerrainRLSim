...
void cOptScenarioPoliEval::SetTimeStep(double time_step) { mTimeStep = time_step; }
...
int cOptScenarioPoliEval::GetPoolSize() const { return static_cast<int>(mEvalPool.size()); }
...
void cOptScenarioPoliEval::EvalHelper(tEvalParams eval_params, std::shared_ptr<cScenarioPoliEval> eval) {
    ...
    while (num_episodes < eval_params.mMaxEpisodes && num_cycles < eval_params.mMaxCycles) {
