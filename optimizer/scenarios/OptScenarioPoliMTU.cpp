...
cOptScenarioPoliMTU::cOptScenarioPoliMTU(cOptimizer &optimizer) : mOptimizer(optimizer) {
    mTimeStep = 1 / 30.0;
...
void cOptScenarioPoliMTU::SetTimeStep(double time_step) { mTimeStep = time_step; }

std::string cOptScenarioPoliMTU::GetName() const { return "Opt Policy MTU"; }
...
void cOptScenarioPoliMTU::ResetParams() { mIter = 0; }
...
void cOptScenarioPoliMTU::OutputMTU(const std::string &output_path) const { mOptScene->OutputSolution(output_path); }
