...
cScenarioSegmentMotion::cScenarioSegmentMotion() { mMirrorClipMode = eMirrorClipNone; }
...
void cScenarioSegmentMotion::Run() { SegmentMotion(mOutputPath); }

std::string cScenarioSegmentMotion::GetName() const { return "Segment Motion"; }
...
void cScenarioSegmentMotion::FindCutFrames(const cMotion &motion, const std::vector<double> &cut_times,
    ...
    for (int f = 0; f < num_frames; ++f) {
        ...
        if (std::abs(frame_time - curr_cut_time) < time_tol || curr_cut_time <= frame_time) {
...
void cScenarioSegmentMotion::CutMotion(const cKinCharacter &kin_char, std::vector<int> &cut_indices,
                                       const std::string &motion_file, const std::string &out_path) {
    ...
    for (int c = 0; c <= num_cuts; ++c) {
        ...
        bool mirror_stance =
            (mMirrorClipMode == eMirrorClipEven && (c % 2 == 0)) || (mMirrorClipMode == eMirrorClipOdd && (c % 2 == 1));
        if (mirror_stance) {
...
void cScenarioSegmentMotion::ExtractMotionClip(const cKinCharacter &kin_char, int start_idx, int end_idx,
