...
class cScenarioSegmentMotion : public cScenario {
  ...
  protected:
    enum eMirrorClip { eMirrorClipNone, eMirrorClipOdd, eMirrorClipEven, eMirrorClipMax };
    ...
    virtual void FindCutFrames(const cMotion &motion, const std::vector<double> &cut_times,
                               std::vector<int> &out_frames);
    virtual void CutMotion(const cKinCharacter &kin_char, std::vector<int> &cut_indices, const std::string &motion_file,
                           const std::string &out_path);
    virtual void ExtractMotionClip(const cKinCharacter &kin_char, int start_idx, int end_idx,
