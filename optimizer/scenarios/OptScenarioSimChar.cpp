...
cOptScenarioSimChar::cOptScenarioSimChar(cOptimizer &optimizer) : cOptScenario(optimizer) {}
...
bool cOptScenarioSimChar::IsThreadSafe() const { return false; }

void cOptScenarioSimChar::SetPoolSize(int size) { mPoolSize = size; }

std::string cOptScenarioSimChar::GetName() const { return "Optimize Sim Char"; }
...
const std::shared_ptr<cScenarioSimChar> &cOptScenarioSimChar::GetDefaultScene() const { return mScenePool[0]; }
...
void cOptScenarioSimChar::FreeScene(int idx) { mPoolManager.FreeIndex(idx); }
