...
cOptScenarioMTU::cOptScenarioMTU(cOptimizer &optimizer) : cOptScenarioSimChar(optimizer) {
    mMaxEpisodes = 1;
    mModelFile = "";
    mCriticModelFile = "";
}
...
bool cOptScenarioMTU::IsThreadSafe() const { return true; }
...
std::string cOptScenarioMTU::GetName() const { return "Optimize MTU"; }
...
int cOptScenarioMTU::GetNumTargetMTUs() const { return static_cast<int>(mTargetMTUIDs.size()); }

void cOptScenarioMTU::ParseMirrorParams(const std::shared_ptr<cArgParser> &parser,
                                        std::vector<tMirrorParam> &out_params) const {
