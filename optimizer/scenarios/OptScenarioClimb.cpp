...
cOptScenarioClimb::cOptScenarioClimb(cOptimizer &optimizer) : cOptScenarioTrackMotion(optimizer) {}

cOptScenarioClimb::~cOptScenarioClimb() {}

std::string cOptScenarioClimb::GetName() const { return "Optimize Climb"; }
...
double cOptScenarioClimb::CalcCurrErr(const std::unique_ptr<cScenarioSimChar> &scene) {
    ...
    err = pose_err_w * pose_err + effort_w * effort + landing_err_w * landing_err + com_err_w * com_err +
          vel_err_w * vel_err;
    ...
    err += y_err + head_vel_err * 0 + front_force_er * 0;
