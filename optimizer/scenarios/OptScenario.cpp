...
cOptScenario::cOptScenario(cOptimizer &optimizer) : mOptimizer(optimizer) {
    mOutputPath = "";
    mIntOutputPath = "";
}
...
void cOptScenario::Init() { cScenario::Init(); }
...
void cOptScenario::Reset() { cScenario::Reset(); }
...
const std::string &cOptScenario::GetOutputPath() const { return mOutputPath; }

void cOptScenario::SetOutputPath(const std::string &path) { mOutputPath = path; }

const cOptFunc::tPoint &cOptScenario::GetSolution() const { return mSoln; }

void cOptScenario::OutputSolution(const std::string &path) const { OutputPt(path, mSoln); }
