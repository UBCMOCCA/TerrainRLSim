...
void cOptCMA::SetCMAParams(const tCMAParams& params)
{
	mParams = params;
}
...
bool cOptCMA::CheckTermination()
{
	int curr_gen = GetCurrGen();
	double sigma = GetCurrSigma();
	bool is_end = curr_gen >= mParams.mMaxGen || mCMA.testForTermination()
...
void cOptCMA::OutputPt(cOptFunc::tPoint& pt, double val, const std::string& filename, const std::string& int_filename_noext) const
{
	if (filename != "")
	{
